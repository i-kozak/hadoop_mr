...
if [[ -z "$INPUT_PATH" ]];
then
  INPUT_PATH="/flight_delays/input/2015_Flight_Delays_and_Cancellations"
fi

if [[ -z "$OUTPUT_PATH" ]];
then
  OUTPUT_PATH="/flight_delays/output"
fi
...
SUBMIT_CMD="${EXECUTOR} jar /usr/lib/hadoop-mapreduce/hadoop-streaming-2.10.2.jar ${mapReduceArguments[@]}"
echo "$SUBMIT_CMD"
${SUBMIT_CMD}
